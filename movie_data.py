...
class MoviesData:
    ...
    def download_content(self,link,movie_name,extension,is1080=False):
        ...
        if self.download:
            ...
            if link:
                img = get(link)
            _create_directory('movies/{}'.format(movie_name))
            if not is1080:
                with open('movies/{}/{}.{}'.format(movie_name,movie_name,extension),'wb') as f:
                    f.write(img.content)
            else:
                with open('movies/{}/{}_1080.{}'.format(movie_name,movie_name,extension),'wb') as f:
                    f.write(img.content)
        ...
        for movie_name, movie_link in zip(movie_names, movie_links):
            if 'yts' in movie_link:
                if not db_conn.find_one({'yts.movie_name':movie_name}):
                    print(movie_name)
                    self.get_movie_page(movie_link,sess)
...
movie_obj = MoviesData(domain,True)
movie_obj.get_page('/browse-movies') 
